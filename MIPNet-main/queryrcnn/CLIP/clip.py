...
def encode_text_with_prompt_ensemble(model, texts, device, prompt_templates=None, template_form="noun"):
    # adjust from the default prompt templates for ImageNet
    if prompt_templates is None:
        if template_form == "noun":
            ...
            prompt_templates = ['a bad photo of a {} scene.',
                                'a low resolution photo of the {} cityscape.',
                                'a bad photo of the {} city.',
                                'a cropped photo of the {} outdoor scene.',
                                'a photo of my {} city.', 'a photo of the cool {} scene.',
                                'a close-up photo of a {} scene.', 'a black and white photo of the {} scene.',
                                'a pixelated photo of the {} scene.',
                                'a bright photo of the {} scene.', 'a cropped photo of a {} scene.',
                                'a photo of the dirty {} scene.', 'a jpeg corrupted photo of a {} city scene.',
                                'a photo of the {} city.', 'a good photo of the {} scene.',
                                'a close-up photo of the {} scene.', 'a photo of a {} scene.',
                                'a low resolution photo of a {} scene.', 'a {} photo',
                                'a photo of a nice {} scene.', 'a photo of a weird {} scene.',
                                'a pixelated photo of a {} outdoor scene.', 'a jpeg corrupted photo of the {} scene.',
                                'a good photo of a {} scene.', 'a photo of the nice {} scene.',
                                'a black and white photo of a {} scene.', 'a dark photo of a {} scene.']
    ...
    for t in texts:
        ...
        for item in t:
            prompted_t.extend([template.format(item) for template in prompt_templates])
        prompted_t = tokenize(prompted_t).to(device)
        class_embeddings = model.encode_text(prompted_t)
        class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
        class_embedding = class_embeddings.mean(dim=0)
...
# def clip_feature_surgery(image_features, text_features, redundant_feats=None):
...
#         b, n_t, n_i, c = image_features.shape[0], text_features.shape[0], image_features.shape[1], image_features.shape[
#             2]
...
def clip_feature_surgery(image_features, text_features, redundant_feats=None):
    print("=== Entering clip_feature_surgery ===")
    print(f"image_features shape: {image_features.shape}")  # (b, n_i, c)
    print(f"text_features shape: {text_features.shape}")  # (n_t, c)")
    if redundant_feats is not None:
        print("redundant_feats provided.")
        print(f"redundant_feats shape: {redundant_feats.shape}")

        # 计算 similarity
        similarity = torch.matmul(image_features, (text_features - redundant_feats).t())
        print(f"similarity after matmul (if redundant_feats is not None): {similarity}")
    else:
        print("redundant_feats is None, proceeding with else branch.")

        # 计算权重
        prob = torch.matmul(image_features[:, :1, :], text_features.t())  # (b, 1, n_t)
        print(f"prob shape: {prob.shape}")
        print(f"prob before softmax: {prob}")

        prob = (prob * 2).softmax(dim=-1)  # (b, 1, n_t)
        print(f"prob after softmax: {prob}")

        w = prob / prob.mean(dim=-1, keepdim=True)  # (b, 1, n_t)
        print(f"w shape: {w.shape}")
        print(f"w: {w}")

        # 计算元素级乘积特征
        b, n_i, c = image_features.shape[0], image_features.shape[1], image_features.shape[2]
        n_t = text_features.shape[0]
        print(
            f"Batch size (b): {b}, Number of text features (n_t): {n_t}, Number of image features (n_i): {n_i}, Channels (c): {c}")

        feats = image_features.view(b, n_i, 1, c) * text_features.view(1, 1, n_t, c)  # (b, n_i, n_t, c)
        print(f"feats shape after element-wise multiplication: {feats.shape}")
        print(f"feats sample: {feats[0, 0, :2, :]}")  # 打印部分样本

        feats *= w.view(b, 1, n_t, 1)  # (b, n_i, n_t, c)
        print(f"feats shape after applying weights w: {feats.shape}")
        print(f"feats after applying weights sample: {feats[0, 0, :2, :]}")

        redundant_feats = feats.mean(dim=2, keepdim=True)  # (b, n_i, 1, c)
        print(f"redundant_feats shape: {redundant_feats.shape}")
        print(f"redundant_feats sample: {redundant_feats[0, 0, 0, :]}")

        if n_t > 1:
            feats = feats - redundant_feats  # (b, n_i, n_t, c)
            print(f"feats shape after subtracting redundant_feats: {feats.shape}")
            print(f"feats after subtraction sample: {feats[0, 0, :2, :]}")
        else:
            print("n_t=1, skipping subtraction of redundant_feats.")
            # 根据需求决定是否需要其他操作
            # 例如，保留feats不变，或者设置为某个默认值
            # 这里我们选择保留feats不变
            pass

        # 计算 similarity
        similarity = feats.sum(dim=-1)  # (b, n_i, n_t)
        print(f"similarity shape: {similarity.shape}")
        print(f"similarity sample: {similarity}")

    print("=== Exiting clip_feature_surgery ===\n")
    return similarity
